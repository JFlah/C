...
String * build_string(char *arr) {
	String *rs;
	
	rs = (String *)malloc(sizeof(String)); // put on heap
	
	int size = strlen(arr);
	
	rs->len=size; // also (*rs).size works
	char *myText = (char *)malloc(size); // must set pointer first to give to rs
	strcpy(myText, arr);
	rs->chars=myText;
